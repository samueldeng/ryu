...
class SDNswitch(app_manager.RyuApp):
	...
	self.addmeter={}
        ...
        self.sleeptime=self.link_num*2
...
##            datapath.send_msg(out)
	...
	print "666666666666666666666666666666666666666"
	print "666666666666666666666666666666666666666"
	print "self.link = ", len(self.links)
...
# Update G when New LLDP happens
# input should be ev.msg

    def update_topology_edge_add(self,msg):
        try:
        ...
        switch_src_num = self.dpids_to_nums[src_dpid]
        switch_dst_num = self.dpids_to_nums[dst_dpid]

        # Check if the link not exist then we add it
        if switch_dst_num not in self.G[switch_src_num].keys():
            self.G[switch_src_num][switch_dst_num] = 1
        if switch_src_num not in self.G[switch_dst_num].keys():
            self.G[switch_dst_num][switch_src_num] = 1


    def update_topology(self, dpid, port_num):
        # Ryu delete link is slow, we fix this by directly delete the links suffering from the port down
        # Port up is not handled
	self.topology()
        dpid_list = self.dpids_to_nums.keys()
        for dpid_dst in dpid_list:
            if (dpid, dpid_dst) in self.dpid_to_port.keys():
                port_pair = self.dpid_to_port[(dpid,dpid_dst)]
                if port_pair[0] == port_num:
                    # src_port meet, link should be delete, from G, dpid_to_port
                    switch_src_num = self.dpids_to_nums[dpid]
                    switch_dst_num = self.dpids_to_nums[dpid_dst]
                    if switch_dst_num in self.G[switch_src_num].keys():
                        self.G[switch_src_num].pop(switch_dst_num)
                       # self.dpid_to_port.pop( (dpid,dpid_dst) )
                    if switch_src_num in self.G[switch_dst_num].keys():
                        self.G[switch_dst_num].pop(switch_src_num)
                       # self.dpid_to_port.pop( (dpid_dst,dpid) )
...
##                self.shortnopacketout(ev, dst, src, self.G)
            ...
            flag1=0
            flag2=0
    	    self.update_topology( dpid, msg.desc.port_no)
...
#        self.topology()
#trying to fix!!!
        self.update_topology_edge_add(ev.msg)
...
##            flag=1#TODO
	...
	print '11111111111111111111111111111111111111111111111'
	    ...
	    print "3333333333333333333333333333333333333333"
            self.topology()
	    print "self.edgenum=", self.edgenum
	    print "self.link_num", self.link_num
            if self.edgenum!=self.link_num:
