...
half = 1#the parameter of whether the FatTree is only a half
...
def fattree(k, half, dic_label_to_port):
    if half == 0:
        ...
        print>>blueprint, str((k * k / 2) + (k * k / 8)) + ' ' + str((k / 2) * (k / 2) * (k / 2) +  k ) + ' 1'#TODO it may only suit to fattree(4)
        ...
        #TODO the half topology is different from half fattree
            j = (i - k * k / 4) % (k / 2)  + 2 * (k * k / 4)
            print>> blueprint, str(i) + ' ' + str(j)
...
class DataCenterRouter(app_manager.RyuApp):
    ...
    def topology_change_handler(self, ev):
    	LOG.debug(' start topology collecting ')
        self.switches = get_switch(self)
        self.links = get_link(self)
	LOG.debug('show switches of topology')
        ...
        n = len(self.switches)
        m = len(self.links)
        print str(n) + ' ' + str(m / 2+m%2) + ' 1'
        print>> devicegraph, str(n) + ' ' + str(
            m / 2+m%2) + ' 1'#TODO self.links seems two-way e.g.,(1,0),(0,1) , but if it may have the possible of only (1,0) ????
        for i in xrange(n):
            graph.append([])
            for j in xrange(n):
                graph[i].append(0)
	    ...
	    print 'link:'+str(srcnum)+' '+str(dstnum)
       ...
       # etac(blueprint, devicegraph)#2014TODO
        ...
        while 1:
           ...
           # self.dpid_to_label[self.nums_to_dpids[int(numline[0])]] = int(numline[1])#2014TODO
            #self.label_to_dpid[int(numline[1])] = self.nums_to_dpids[int(numline[0])]#2014TODO
       ...
       # blueprint_ports_to_devicegraph_ports(self.bports_to_dports, self.label_to_port, self.dpid_to_port,self.dpid_to_label)#2014TODO
        #end etac
        LOG.debug('show switches of topology')
        LOG.debug(self.switches)
        LOG.debug('\n\n\n\n\n\n')
